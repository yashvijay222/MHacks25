...
export class ArrowsSpawner extends BaseScriptComponent {
    ...
    /**
     * Start showing navigation arrows along the path
     */
    start(arrowsPositions: vec3[], mySplinePoints: {position: vec3, rotation: quat}[], myPathLength: number) {
        ...
        // Clear some points from beginning and end to avoid arrows at start/finish
        const offset = 7;
        ...
        log.i(`Started arrows spawner with ${this.positions.length} positions`);
    }

    /**
     * Stop showing navigation arrows
     */
    stop() {
        this.isActive = false;
        this.updateEvent.enabled = false;
        this.clearState();
        log.i("Stopped arrows spawner");
    }

    /**
     * Clear all spawned arrows
     */
    clearState() {
    ...
    private getSpawnedArrowsCount(): number {
        return this.spawnedArrows.length;
    }

    private getArrowsLimit(): number {
        return this.maxArrows;
    }

    private tryGetNextArrowPositionAndRotation(): vec3 | null {
    ...
    private tryGetOldestArrowPositionAndRotation(): SpawnedArrowData | null {
    ...
    private removeOldestArrow() {
        ...
        this.spawnedArrows[0].objects.forEach((obj) => obj.destroy());
        this.spawnedArrows.shift();
    }

    private getDistanceOnSpline(posA: vec3, posB: vec3): number {
        // Simplified distance calculation for route navigation
    ...
    private incrementNextId(currentPosition: vec3) {
        const currentOldest = this.tryGetOldestArrowPositionAndRotation();
        if (!currentOldest) {
            throw new Error("Cannot increment next id. Current oldest is null.");
        }
        const currentOldestId = currentOldest.id;
        while (this.nextId < this.positions.length && 
    ...
    private spawnNextArrow() {
        ...
        const {leftArrow, rightArrow} = this.instantiateSideArrowObj(currentPosition, rot);
    ...
    private instantiateSideArrowObj(position: vec3, rotation: quat) {
        ...
        const right = rotation.getRight();
        const up = rotation.getUp();
        ...
        // Apply material to spawned arrow
        if (this.arrowMaterial) {
            const renderMesh = leftArrow.getChild(0).getChild(0).getComponent("RenderMeshVisual");
            if (renderMesh) {
                renderMesh.mainMaterial = this.arrowMaterial;
            }
        }
        ...
        // Apply material to spawned arrow
        if (this.arrowMaterial) {
            const renderMesh = rightArrow.getChild(0).getChild(0).getComponent("RenderMeshVisual");
            if (renderMesh) {
                renderMesh.mainMaterial = this.arrowMaterial;
            }
        }
    ...
    private getPlayerPosition(): vec3 {
        ...
        return this.mainCameraT.getWorldPosition();
    }
}
